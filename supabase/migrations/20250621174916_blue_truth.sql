/*
  ...
  1. Changes
     - Drops existing ambiguous lock_funds functions
     - Creates a single, well-defined lock_funds function with consistent parameter order
     - Adds proper error handling and validation
     - Returns JSON response with success/failure status

  2. Security
     - Uses SECURITY DEFINER to ensure function runs with creator's permissions
