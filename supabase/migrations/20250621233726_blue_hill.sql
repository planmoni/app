/*
  ...
  1. Problem
    - The wallet balance in the database doesn't match the available balance shown in the UI
    - The process_payout function doesn't properly update the wallet balance
    - This causes a discrepancy between the database balance and the UI calculated balance
  
  2. Solution
    - Update the process_payout function to properly decrease both balance and locked_balance
    - This ensures the wallet balance accurately reflects completed payouts
*/

-- Create or replace the process_payout function with proper wallet balance updates
...
BEGIN
  ...
  -- THIS IS THE FIX: Update wallet balance and locked_balance
