...
CREATE POLICY IF NOT EXISTS "Anyone can read email verification status" 
...
CREATE POLICY IF NOT EXISTS "Service role can manage all email verification records" 
  ON public.email_verification_cache
  FOR ALL 
  TO service_role
  USING (true)
...
CREATE POLICY IF NOT EXISTS "Users can update their own email verification" 
  ON public.email_verification_cache
  FOR UPDATE 
  TO authenticated
...
CREATE POLICY IF NOT EXISTS "Users can insert their own email verification" 
  ON public.email_verification_cache
  FOR INSERT 
  TO authenticated
  WITH CHECK ((auth.uid())::text = email);
