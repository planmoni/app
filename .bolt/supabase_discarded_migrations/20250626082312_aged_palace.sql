/*
  ...
  1. New Functions
    - `send_otp_email`: Generates a 6-digit OTP, stores it in the database, and sends an email using Supabase's SMTP server
    - `verify_otp`: Verifies the OTP against what's stored in the database and marks it as used

  2. Security
    - Both functions are SECURITY DEFINER to ensure they run with elevated privileges
